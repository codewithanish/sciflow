...
const weightEvents = (data: TeamData) => {
    ...
    const studentWeights = students.map((student) => {
      ...
      const weights = events.map((event) => {
        ...
        if (
          ...
          const { placement, weight } = testOffMap
            .get(studentName)!
            .get(event)!;
  ...
  function assignTasks(agents: Agent[], tasks: string[], teamName: string): Assignment[] {
    const taskAssignments: Assignment[] = [];
    const taskCount = tasks.length;

    // Min-heap for each task to track best agents
    const taskMinHeaps: Map<number, MinHeap<{ student: string; weight: number }>> = new Map();
    
    for (let i = 0; i < taskCount; i++) {
        const minHeap = new MinHeap<{ student: string; weight: number }>((a, b) => a.weight - b.weight);
    ...
    for (let i = 0; i < taskCount; i++) {
        ...
        while (bestAgents.length < requiredAgents && !minHeap.isEmpty()) {
            ...
            agentAssignments.set(candidate.student, (agentAssignments.get(candidate.student) || 0) + 1);
        }
        ...
        while (bestAgents.length < requiredAgents) {
            ...
            bestAgents.push({ student: fallback.student, weight: fallback.weights[i] });
            agentAssignments.set(fallback.student, (agentAssignments.get(fallback.student) || 0) + 1);
        }
  ...
  export async function POST(req: NextRequest) {
    try {
      const { data, teamCount } = await req.json();
  
      if (!data || !teamCount) {
        return NextResponse.json({ error: "Missing data or teamCount" }, { status: 400 });
      }
  
      const weightedEvents = weightEvents(data);
      const assignments = assignTasksToMultipleTeams(weightedEvents, data.events, teamCount);
